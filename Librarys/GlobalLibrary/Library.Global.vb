...
Imports System.IO
Imports Lean.Test.Automation.Framework.LibrarySeleniumRC
Imports Lean.Test.Automation.Framework.LibrarySeleniumWDBrowserStack
...
Module variables
    ...
    Public p_ServerWD As String '= "D:\00 Fabrica de Teste\00 Automação\PCFramework\API\IEDriver"
...
Namespace LibraryGlobal
    Public Class CallTest
        ...
        Public Shared Function RunTest(ScriptName As String) As Boolean
            LogExecution("RunTest: " & ScriptName, StateLog.Start)
            Try
                ...
                LogExecution("RunTest: " & ScriptName, StateLog.Finish)
                Return Value
            Catch ex As Exception
                LogExecution("RunTest error: " & ex.Message, StateLog.Exception)
                MsgBox("RunTest " & ScriptName & "' error: " & ex.Message.ToString)
                Return False
            End Try
            ' executar scripts em arquivos
            'Dim file = New FileTexts
            '   Dim scr As String = file.ReadFile("C:\LeanTestAutomation\Scripts\FilesText\" & ScriptName & ".txt")
            '   Dim dc = New DynaCode.Program
            '   Dim args As String() = {scr}
            'dc.CompileAndRun(args, ScriptName, DynaCode.Program.Language.VisualBasic)
            'para isso funcionar e necessario retirar todos os shares dos metodos e usar nos scripts test.endtest or exemplo
    ...
    Public Class LibGlobal
        ...
        Public Enum typeIdentification
            ...
            leanTest
        End Enum
        Private librarySeleniumRCInteraction As New LibrarySeleniumRC.InteractionRC
        Private librarySeleniumWDInteraction As New LibrarySeleniumWD.InteractionWD
        Private librarySeleniumWDBSInteraction As New LibrarySeleniumWDBrowserStack.InteractionWDBrowseerStack
        Private libraryAppiumInteration As New LibraryAppium.InteractionAppium
        'Private libraryVisualStudioCodedUI As New LibraryVisualStudioCodedUI.InteractionVS
        ...
        Function StartTests(Optional noReset As String = "", Optional waitActivity As String = "") As Boolean
            ...
            Try
                Select Case p_ToolName
                    Case ActiveTool.SeleniumRC 'selenium remote control
                        Dim rc = New LibrarySeleniumRC.SeleniumRCHelper
                        Process.Start(p_pathSeleniuRC) 'path selenium rc server
                        rc.SetupTest()
                        objSeleniumRC.WindowMaximize()
                        LogExecution("StartTests: SeleniumRC", StateLog.Finish)
                        Return True
                    Case ActiveTool.SeleniumWD 'selenium web driver
                        ...
                        Return True
                    Case ActiveTool.SeleniumWDBrowserStack 'selenium wedrive in clound
                        Dim wd = New LibrarySeleniumWDBrowserStack.WDBrowserStackHelper
                        'Process.Start(p_pathSeleniuRC) 'path selenium rc server
                        wd.SetupTest()

                        objSeleniumWDBS.Manage.Window.Maximize()
                        LogExecution("StartTests: SeleniumWDBrowserStack", StateLog.Finish)
                        Return True
                    Case ActiveTool.VisualStudioCodedUI

                        Return True
                    Case ActiveTool.SilkTest
        ...
        Shared Sub EndScenario(Optional idSceanario As Integer = 0, Optional scenarioName As String = Nothing)
            ...
            Try
                ...
                p_StatusScenario = Nothing 'limpa a variavel para o proximo cenario
        ...
        Sub LoadVariables()
            ...
            p_timeout = pc_db.GetParameter("TimeOut") '10 'seconds
            ...
            Try
                ...
                Select Case p_ToolName
                    ...
                    Case ActiveTool.SeleniumWDBrowserStack
                        'librarySeleniumWDBSInteraction.Open(url)
                    Case ActiveTool.SeleniumWD
                        librarySeleniumWDInteraction.GetAttribute(element, attribute, typeIdentification)
                    Case ActiveTool.Appium
                        'libraryAppiumInteration.Open(url)
                    Case Else
        ...
        Function DoubleClick(ImgPathOrCoordinatesXY As String, value As Boolean, Optional typeIdentification As typeIdentification = typeIdentification.xpath, Optional waitElement As Boolean = True) As Boolean
            ...
            Try
                ...
                Else
                    Select Case p_ToolName
                        Case ActiveTool.SeleniumRC
                            librarySeleniumRCInteraction.DoubleClick(ImgPathOrCoordinatesXY)
                        Case ActiveTool.SeleniumWD
                            librarySeleniumWDInteraction.DoubleClick(ImgPathOrCoordinatesXY, typeIdentification)
                        Case ActiveTool.SeleniumWDBrowserStack
                            librarySeleniumWDBSInteraction.DoubleClick(ImgPathOrCoordinatesXY)
                        Case ActiveTool.Appium
